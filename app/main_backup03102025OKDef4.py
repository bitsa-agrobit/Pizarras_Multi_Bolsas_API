...
#
# Cambios "quirúrgicos": fix ORA-01438 para SIGLO y UVALUE (ver export_oracle).
...
_GRAIN_MAP = {"Trigo":1, "Maiz":2, "Soja":3, "Sorgo":4, "Girasol":5, "Trigo Art 12":12}
...
def export_oracle(
    ...
    # === FIX ORA-01438 ===
    siglo = 21                # NUMBER(3) → 21 (siglo XXI). Antes 2000 desbordaba.
    cosecha = "0"             # placeholder conservador
    ...
    try:
        ...
        for it in rows:
            ...
            # NUMBER(16,2) → dos decimales
            precioref = round(precioref, 2)

            uvalue = _uvalue16(grano, siglo, cosecha, pizarra, fechavig, mes, ejercicio, precioref)

            # Idempotente por UVALUE (PRIMARY KEY según tu script)
            cur.execute("""
