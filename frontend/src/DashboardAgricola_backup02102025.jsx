...
/** === Helpers === */
const API = ""; // usamos el proxy de Vite → /api/...
...
const looksLikeFuturo = (name = "") => {
  // contratos, meses/año, abreviaturas típicas, etc.
  return (
...
const dedupeByProducto = (items) => {
  ...
  for (const it of items) {
    ...
    // conservamos el primero que tenga precio numérico; si no, el existente
...
export default function DashboardAgricola() {
  ...
  const fetchData = async (pz = plaza) => {
    setLoading(true);
    try {
      const res = await fetch(`${API}/api/cotizaciones?plaza=${pz}`);
  ...
  const filtered = useMemo(() => {
    ...
    // orden simple: productos con precio primero
    finalList.sort((a, b) => {
  ...
  const onStart = async () => {
    try {
      const res = await fetch(
        `${API}/api/start?plaza=${plaza}&interval_min=${intervalMin}`,
        { method: "POST" }
      );
      const js = await res.json();
      alert(
        js?.ok
          ? "Éxito: Automatización iniciada"
          : `Error al iniciar: ${js?.message || "desconocido"}`
      );
    } catch (e) {
  ...
  const onExportOracle = async () => {
    try {
      const res = await fetch(`${API}/api/export/oracle?plaza=${plaza}`, {
        method: "POST",
      });
      const js = await res.json();
      alert(
        js?.ok
          ? `Éxito: exportadas ${js.exported} filas`
          : `Error al exportar: ${js?.error || "desconocido"}`
      );
    } catch (e) {
