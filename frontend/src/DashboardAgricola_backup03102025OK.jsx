...
/** === Helpers === */
const API = ""; // FIX: usar proxy Vite → /api

// FIX: normalización de plaza (rosario / bahía / locales)
...
// FIX: función limpia (antes se había colado texto)
...
export default function DashboardAgricola() {
  ...
  const [hideSC, setHideSC] = useState(true); // FIX: tildado por defecto
  ...
  const fetchData = async (pz = plaza, t = tab) => {
    setLoading(true);
    try {
      // FIX: only_base según pestaña (base=1, futuros=0) + normalización/encode
  ...
  // Carga inicial y al cambiar plaza o pestaña base/futuros
  useEffect(() => {
    fetchData(plaza, tab);
  ...
  /** KPIs */
  const kpi = useMemo(() => {
    const nums = filtered.map((r) => r.price).filter(isNumeric); // FIX: usa 'price'
